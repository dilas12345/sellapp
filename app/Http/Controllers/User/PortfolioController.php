...
class PortfolioController extends Controller
{
    ...
    public function viewPreview(Request $request, $id)
    {
        ...
        $portfolio_details = DB::table('portfolios')->where('portfolio_id', $id)->where('status', 1)->first();

        if (isset($portfolio_details)) {
            if ($portfolio_details->card_type == "store") {
                ...
                if ($business_portfolio_details) {
                    ...
                    $galleries_details = DB::table('galleries')->where('portfolio_id', $portfolio_details->portfolio_id)->orderBy('id', 'asc')->get();
                    $payment_details = DB::table('payments')->where('portfolio_id', $portfolio_details->portfolio_id)->get();
                    $business_hours = DB::table('business_hours')->where('portfolio_id', $portfolio_details->portfolio_id)->first();
                    $make_enquiry = DB::table('business_fields')->where('portfolio_id', $portfolio_details->portfolio_id)->where('type', 'wa')->first();
    ...
    public function CreatePortfolio()
    {
        ...
        $themes = Theme::where('theme_description', 'Portfolio page')->where('status', 1)->get();
        $settings = Setting::where('status', 1)->first();
        ...
        // dd($plan_details);
	    if($plan_details->no_of_portfolios == 999) {
            $no_portfolios = 999999;
        } else {
            $no_portfolios = $plan_details->no_of_portfolios;
        }
    ...
    public function savePortfolio(Request $request)
    {
        ...
        $current_portfolio = Portfolio::where('porfolio_url', $portfolio_url)->count();
        ...
        if ($current_portfolio == 0) {
            ...
            if ($portfolios < $no_portfolios) {
                try {
                    ...
                    $portfolio->porfolio_url = $portfolio_url;
                    ...
                    return redirect()->route('user.port.social.links', $portfolio_id);
                } catch (\Exception $th) {
                    alert()->error(trans('Sorry, personalized link was already here.'));
                    return redirect()->route('user.create.portfolio');
                }
            } else {
    ...
    public function socialLinks()
    {
        ...
        return view('user.portfolio.social-links', compact('plan_details', 'settings'));
    }
    ...
    public function saveSocialLinks(Request $request, $id)
    {
        ...
        if ($portfolio == null) {
            ...
            if ($request->icon != null) {
                PortBusinessField::where('portfolio_id', $id)->delete();
                $plan = DB::table('users')->where('user_id', Auth::user()->user_id)->where('status', 1)->first();
                $plan_details = json_decode($plan->plan_details);
                ...
                if (count($request->icon) <= $plan_details->no_of_features) {
                    for ($i = 0; $i < count($request->icon); $i++) {
                        if (isset($request->icon[$i]) && isset($request->label[$i]) && isset($request->value[$i])) {
                            ...
                            return redirect()->route('user.port.social.links', $id);
                        }
                    }
                    alert()->success(trans('features details updated'));
                    return redirect()->route('user.port.payment.links', $id);
                } else {
    ...
    public function savePaymentLinks(Request $request, $id)
    {
        ...
        if ($portfolio == null) {
            ...
            if ($request->icon != null) {
                Payment::where('portfolio_id', $id)->delete();
                $plan = DB::table('users')->where('user_id', Auth::user()->user_id)->where('status', 1)->first();
                $plan_details = json_decode($plan->plan_details);
                ...
                if (count($request->icon) <= $plan_details->no_of_payments) {
                    ...
                    return redirect()->route('user.port.services', $id);
                } else {
    ...
    public function services()
    {
        ...
        return view('user.cards.port.services', compact('plan_details', 'settings', 'media'));
    }
        ...
        if ($portfolio == null) {
            ...
            if ($request->service_name != null) {
                ...
                if (count($request->service_name) <= $plan_details->no_of_services) {
                    ...
                    alert()->success(trans('port.services details updated'));
        ...
        if ($Portfolio == null) {
            ...
            if ($portfolio == null) {
                ...
                if ($portfolio->portfolio_status == 'inactive') {
                    ...
                    $no_of_galleries = Gallery::where('portfolio_id', $id)->count();
                    $no_of_payments = Payment::where('portfolio_id', $id)->count();
